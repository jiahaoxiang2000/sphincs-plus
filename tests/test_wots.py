...
class TestWOTS(unittest.TestCase):
    ...
    def test_wots_pk_from_sig(self):
        # Generate the signature
        sig = bytearray(SPX_WOTS_LEN * SPX_N)
        wots_sign(sig, self.msg, self.sk_seed, self.pub_seed, self.addr)

        # Generate the public key from the signature
        pk_from_sig = bytearray(SPX_WOTS_LEN * SPX_N)
        wots_pk_from_sig(pk_from_sig, sig, self.msg, self.pub_seed, self.addr)

        # Generate the original public key for comparison
        original_pk = wots_gen_pk(self.sk_seed, self.pub_seed, self.addr)

        # Verify that the public key generated from the signature matches the original public key
        self.assertEqual(pk_from_sig.hex(), original_pk.hex())
